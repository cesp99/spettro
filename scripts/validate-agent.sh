...
# Check description field — extract multiline value (all lines until next key or end)
DESCRIPTION=$(echo "$FRONTMATTER" | awk '/^description:/{found=1; sub(/^description: */,""); print; next} found && /^[a-z_-]+:/{found=0} found{print}')
...
if [ -z "$DESCRIPTION" ]; then
  ...
  # Check for "Use this agent" pattern
  if ! echo "$DESCRIPTION" | grep -qi 'use this agent'; then
    echo "⚠️  description should start with 'Use this agent when...' or 'Use this agent to...'"
    ((warning_count++))
  fi
fi
